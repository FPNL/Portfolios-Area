...
include_once("conn/_db_Config.php");
    ...
    switch ($action) {
        ...
        $sql_read ="SELECT  tuserinfo.cuserinfo_id AS id, 
                        ...
                        WHERE aa.cnewstitle LIKE '%".$_POST["search_key"]."%'".
        ...
        case "pers": // 更新操作
            ...
            $sql_read ="SELECT  tuserinfo.cuserinfo_id AS id, 
                        ...
                        LEFT JOIN 	ttaginfo ON tnewstaginfo.ctaginfo_id=ttaginfo.ctaginfo_id
                        LEFT JOIN 	tmediainfo ON aa.cnewsinfo_id=tmediainfo.cnewsinfo_id
                        WHERE tuserinfo.cuserinfo_id=".$id."
                        GROUP BY  id, aa.cnewstitle";
                        // echo $sql."<br>";
            break;

        case "del":    // 刪除操作
            $sql = "DELETE FROM tnewsinfo WHERE cnewsinfo_id=".$id;
            mysqli_query($db,$sql); // 執行SQL指令
            header("Location: frm_news_list.php"); // 轉址serach.php
            break;

        case "edit":   // 編輯操作
            $sql = "SELECT * FROM tnewsinfo WHERE cnewsinfo_id=".$id;
        ...
        case "class":
            ...
            LEFT JOIN 	ttaginfo ON tnewstaginfo.ctaginfo_id=ttaginfo.ctaginfo_id
            LEFT JOIN 	tmediainfo ON aa.cnewsinfo_id=tmediainfo.cnewsinfo_id
            WHERE ttaginfo.ctaginfo_id=".$_GET["tags"]."
            GROUP BY  id, aa.cnewstitle";
            // echo $sql."<br>";
            break;
            default:
        ...
        <main> 
            <div id="news_pops_column">
                <div id="news_pop_caption">
                    <img src="img/background-transparent.png" alt="即時新聞" class="transparent_img">
                    <span>
                        新聞列表
                    </span>
                    <span>
                ...
                if ($rowsCount > 0) {
                    while ($row_read = $rows_read->fetch_assoc()) {
                        ...
                        $img_read=$row_read["img"];
                      
                 echo

                "<div class='news_pop'>".  
                    "<div class='news_pop_img'>".
                            "<a href='frm_news_detail.php?action=view&newsid=".$newsid_read."'><img src='upload/".$img_read."' alt='Incorrect path'></a>".
                    "</div>".
                    "<div class='news_pop_main'>".  
                            "<div class='news_class'><a href='frm_news_view.php?action=class&tags=".$tagid_read."'><span class='news_clss_span'>" .$tag_read."</span></a> </div>".    
                            "<div class='news_title'><a href='frm_news_detail.php?action=view&newsid=".$newsid_read."'><span>".$title_read."</span></a> </div>".    
                                ...
                                "<div class='news_icon_bar'>".
                                    "<div class='news_distance news_icon_items'>".
                                        ...
                                        "<div class='news_distance_descr news_descr'>".
                                            "<a href='frm_news_map.php?action=search&X=".$locationY_read."&Y=".$locationY_read."'>".
                                            "<span>500 KM</span></a>".
                                        "</div>".
                                    "</div>".
                                    ...
                                    if($struser_id==""){
                                        ...
                                        echo 
                                    "<div class='news_trim'>".
                                        "<a href='frm_news_edit.php?action=edit&newsid=".$newsid_read."'>".
                                            "<span>修改新聞</span>".
                                        "</a>".
                                    "</div>";
                                    }
                ...
                <div id="recommand_top5">
                    ...
                    <?php
                        $sql_recommand = "SELECT  tuserinfo.cuserinfo_id AS id, 
                                ...
                                ORDER BY rand()
                                LIMIT 5";
                                $rows_recommand = mysqli_query($db,$sql_recommand);
                                // while($row = mysql_fetch_row($result)){
                                while ($row_recommand = $rows_recommand->fetch_assoc()) 
                                echo
